...
function encode (value) {
	...
	if(value < 0) {
		x.writeUInt32BE((~x.readUInt32BE(0)) >>> 0, 0);
		x.writeUInt32BE((~x.readUInt32BE(4)) >>> 0, 4);
	} else {
		/* non-negative; set the leading bit. */
		x.writeUInt8(x.readUInt8(0) | 0x80, 0);
	}
...
encode.decode = function(str) {
	var x, h, l;
	if(str[0] != '\ue2f2') return str;
	x = dec85(str.substr(1)); // Remove the prefix character.
	
	h = x.readUInt8(0);
	if(h & 0x80) {
		// originally a non-negative number. Just set the sign bit back to 0.
		x.writeUInt8(h & 0x7f, 0);
	} else {
		x.writeUInt32BE((~x.readUInt32BE(0)) >>> 0, 0);
		x.writeUInt32BE((~x.readUInt32BE(4)) >>> 0, 4);
	}
