...
module.exports = function(q, cb) {
	...
	function getObjsFromIKeys(keys, cb) {
		var os = [], ds = [], fc = 0, order = [], i, l, error; // ObjectS, (link)Data(S) and FetchCount
		
		function getObjFromIKeyIx(ix) {
			var link, key = keys[ix], id;
			if(error) return;
			key = key.split(flds);
			id = key[key.length-2];
			...
			fc++;
			getObjFromDKey(self.getDataKey(id), function(err, obj) {
				if(err) error = err;
				else os[ix] = obj;
				done();
			});
			...
			if(link) {
				fc++;
				getObjFromDKey(link.getDataKey(id, key[1]), function (err, obj) {
					if(err) error = err;
					else ds[ix] = obj;
					done();
				});
	...
	function getIKeysFromQuery(q, cb) {
		...
		db.createReadStream({
			...
			lte: Buffer.concat([Buffer(ent.getIndexKey(by, q.lte)), Buffer([0xff])]),
