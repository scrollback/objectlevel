...
module.exports = function(q, cb) {
	var key, err, res=[], rel={}, getc=0, 
		type = this.type, opt = this.opt, link, relq,
	...
	if(typeof q !== 'object') {
		db.get(type + flds + q + flds, function(err, range) {
		...
		if(q.gte && !(q.gte instanceof Array)) q.gte = [q.gte];
		if(q.lte && !(q.lte instanceof Array)) q.lte = [q.lte];
		...
		getc++;
		db.createReadStream({
			gte: Buffer(key + (q.gte? this.db.key(q.gte): '') + flds),
			lte: Buffer.concat([Buffer(key + (q.lte? this.db.key(q.lte): '') + flds), Buffer([0xff])]),
			values: q.values, keys: q.keys, reverse: q.reverse, limit: q.limit
		}).
		...
		if(link && !q.keys) {
			...
			on('data', function(data) {
				rel[data.key.split(flds).slice(-2,-1)] = data.value;
			}).
			on('error', function(e) { err = e; }).
			on('close', results);
		}
	...
	function results() {
		...
		if(q.keys) {
			for(i=0, l=res.length; i<l; i++) {
				ret[i] = res[i].split(flds).slice(1, -1).map(decode);
			}
			return cb(null, ret);
		}
		for(i=0, l=res.length; i<l; i++) {
			fetc++;
			files.get(res[i], type, gotobj);
		}
