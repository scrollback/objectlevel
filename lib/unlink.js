...
module.exports = function(from, rel, to, cb) {
	...
	function addrel(rel) {
		...
		db.createReadStream({ 
			...
			if(!to || cto == to) addLink(from, rel, key.slice(2, -2), to);
		}).on('close', exec);
	}
	...
	function addLink(from, rel, key, to) {
		...
		batch.push({type: 'del', key: type + hdrs + rel + flds + to + key + from + flds });
		batch.push({ type: 'del', key: link.type + hdrs + link.rel + flds + from + key + to + flds });
		
		/* Delete the link data */			
		batch.push({type:'del', key: type+hdrs+rel+hdrs+link.type+hdrs+link.rel + flds + from + flds + to + flds });
		batch.push({type:'del', key: link.type+hdrs+link.rel+hdrs+type+hdrs+rel + flds + to + flds + from + flds });
	}
