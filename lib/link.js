...
module.exports = function(from, rel, to, data, cb) {
	...
	link = opt.links[rel];
	tokey = link.type + flds + to + flds; /* key of the object to which this link is made, already existing */
	fromkey = type + flds + from + flds; /* key of the object from which this link is makde, already existing */
	...
	odatakey = ot + hdrs + it + flds + from + flds + to + flds;
	idatakey = it + hdrs + ot + flds + to + flds + from + flds;
	...
	function gotranges() {
		...
		batch.push({type:'put', key: ot + flds + to + flds + from + flds, value:fromrange });
		batch.push({type:'put', key: it + flds + from + flds + to + flds, value:torange });
		...
		if(link.indexes) {
			...
			for(i=0, l=ino.length; i<l; i++) {
				if(!oin[ino[i]]) {
					batch.push({type:'put', key: ot + flds + to + flds + ino[i] + flds + from + flds, value:fromrange });
					batch.push({type:'put', key: it + flds + from + flds + ino[i] + flds + to + flds, value:torange });
				}
				nin[ino[i]] = true;
			}
			...
			if(ind) for(i=0, l=ind.length; i<l; i++) {
				if(!nin[ind[i]]) {
					batch.push({type:'del', key: ot + flds + to + flds + ind[i] + flds + from + flds });
					batch.push({type:'del', key: it + flds + from + flds + ind[i] + flds + to + flds });
				}
			}
